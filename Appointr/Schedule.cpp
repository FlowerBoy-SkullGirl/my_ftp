...
//Finds the nth iterator of a vector, returns end() on failure
vector<Appointment>::iterator Schedule::appt_at(int subscript)
{
	int i = subscript;
...
vector<Appointment>::iterator Schedule::find(Appointment appt)
{
...
vector<Appointment>::iterator Schedule::container_begin()
{
	return schedule.begin();
}

vector<Appointment>::iterator Schedule::container_end()
{
	return schedule.end();
}
